...
import { getSchemasAsText } from "./schemaReader";

const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY!);

// Tool definition ให้ AI รู้จัก
const tools: Tool[] = [
  {
    functionDeclarations: [
      // ประกาศ tool "query" ที่ AI จะใช้เรียกเพื่อดึงข้อมูลจาก database
      {
        name: "query",
...
export async function askAI(userMessage: string): Promise<string> {
  const schemaText = getSchemasAsText();
  ...
  while (response.response.functionCalls()?.length) {
    ...
    const results = await Promise.all(
      calls.map(async (call) => {
        ...
        const result = await runQuery({ ...args, model: args.model as string, action: args.action as string });
        return {
          functionResponse: {
            name: call.name,
            response: { result },
